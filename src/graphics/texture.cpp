...
CubeTexture::CubeTexture(const util::fs::path &fileName) {
    stbi_set_flip_vertically_on_load(false);
    std::string temp = fileName.string();
    std::array<const char *, 6> cstringPath;
    for (int i = 0; i < 6; ++i) {
        cstringPath[i] = temp.c_str();
    }
...
CubeTexture::CubeTexture(const std::array<util::fs::path, 6> &fileName) {
    stbi_set_flip_vertically_on_load(false);
    std::array<std::string, 6> stdstringPath;
    std::array<const char *, 6> cstringPath;
    for (int i = 0; i < 6; ++i) {
        stdstringPath[i] = fileName[i].string();
        cstringPath[i] = stdstringPath[i].c_str();
    }
    loadTexture(cstringPath);
}
